...
public class JpqlCompanySaveAndFetchTest  extends ORMStandaloneClassTestCase{
    ...
    @Test
    public void shouldRetriveAllCompany(){
        Session session = sf.openSession();
        try {
         session.createQuery("SELECT c FROM "+Company.class.getSimpleName()+" c ").list().forEach(c->log.info("+++  {}",c));
        } catch (Exception e) {
            log.error("{}", e);
        } finally {
            session.close();
    ...
    @Test
    public void shouldRetriveRisco(){
        Session session = sf.openSession();
        try {

         List<Company> companies = session.createQuery("SELECT c FROM "+Company.class.getSimpleName()+" c ").list();
         Optional<Company> risco = companies.stream().filter(c->c.getName().equals("risco")).findAny();
         Assertions.assertThat(risco.get().getDepts().size()).isEqualTo(3);
        } catch (Exception e) {
            log.error("{}", e);
        } finally {
            session.close();
    ...
    @Test
    public void shouldAllPersonWhoWorkInJavaDept(){
        Session session = sf.openSession();
        try {
         List<Employee> employees = session.createQuery("SELECT c FROM "+Employee.class.getSimpleName()+" e join e.company c join c.depts d WHERE d.name = :name").
                 setParameter("name", "JAVA").list();
        assertThat(employees.stream().count()).isEqualTo(7l);
    ...
    @Test
    public void shouldGetAllPersonAndCompanyWhoWorkInJavaTechnology(){
        Session session = sf.openSession();
        try {
         List<Object[]> employees = session.createQuery("SELECT c.name,e.lastName FROM "+Employee.class.getSimpleName()+" e join e.company c join c.depts d WHERE d.name = :name").
                 setParameter("name", "JAVA").list();
        for(Object[] o : employees){
            log.info("company : {} , name : {}",o[0].toString(), o[1].toString());
        }
