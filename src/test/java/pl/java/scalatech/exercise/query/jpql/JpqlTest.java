...
public class JpqlTest extends ORMStandaloneClassTestCase{
    ...
    public static  void populateDB(){
        ...
        Company assecco = Company.builder().address(Address.builder().city("Sopot").street("mechowa").zipcode("25664").build()).depts(Lists.newArrayList(qaDepartment,uatDepartment,hrDepartment)).name("assecco").build();
        ...
        Company commarch = Company.builder().address(Address.builder().city("Cracow").street("motorowa").zipcode("24235").build()).depts(Lists.newArrayList(qaDepartment,uatDepartment,javaDepartment,hrDepartment)).name("commarch").build();
        Company ibm = Company.builder().address(Address.builder().city("LA").street("oakStreet").zipcode("23231").build()).depts(Lists.newArrayList(qaDepartment,uatDepartment,csDepartment,hrDepartment,javaDepartment)).name("ibm").build();
    ...
    public void shouldRetriveAllWhereWorkInDEPT(){
        Session session = sf.openSession();
        try {
         session.createQuery("SELECT e.lastName ,c.name , d.name FROM "+Employee.class.getSimpleName() +" e join e.company c join c.depts d  WHERE  d.name =  :name").setParameter("name","JAVA").list().forEach(c->log.info("+++  {}",c));
